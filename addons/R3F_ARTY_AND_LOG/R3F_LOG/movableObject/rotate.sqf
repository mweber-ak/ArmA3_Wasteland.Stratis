...
if (R3F_LOG_mutexLocalLock) then {
	player globalChat STR_R3F_LOG_mutexActionOngoing; // French crap
} else {
	_targetDirection = (getDir R3F_LOG_playerMovesObject) + _rotateAmount; // Get the direction of the object and increment by _rotateAmount
	_targetDirection = _targetDirection - getDir player;

	R3F_LOG_playerMovesObject setDir _targetDirection;
	R3F_LOG_FRAC_PUBVAR_setDir = [R3F_LOG_playerMovesObject, _targetDirection];
